...
{
    /// <summary>
    /// WebPFormat 另一个库，也可以使用 Wrapper for WebP format in C#. (GPL) Jose M. Piñeiro
    /// 需要有 libwebp_x86.dll libwebp_x64.dll 文件在运行目录下
    /// <code>
    /// 解压缩函数:
    /// Bitmap <see cref="Load"/>(string pathFileName) - 以 Bitmap 形式加载WebP文件.
    /// Bitmap <see cref="Decode"/>(byte[] rawWebP) - Decode WebP data (rawWebP) to bitmap.
    /// Bitmap <see cref="Decode"/>(byte[] rawWebP, WebPDecoderOptions options) - Decode WebP data (rawWebP) to bitmap using 'options'.
    /// Bitmap <see cref="GetThumbnailFast"/>(byte[] rawWebP, int width, int height) - Get a thumbnail from WebP data (rawWebP) with dimensions 'width x height'. Fast mode.
    /// Bitmap <see cref="GetThumbnailQuality"/>(byte[] rawWebP, int width, int height) - Fast get a thumbnail from WebP data (rawWebP) with dimensions 'width x height'. Quality mode.
    /// 
    /// 压缩函数:
    /// <see cref="Save"/>(Bitmap bmp, string pathFileName, int quality = 75) - Save bitmap with quality lost to WebP file. Opcionally select 'quality'.
    /// byte[] <see cref="EncodeLossy"/>(Bitmap bmp, int quality = 75) - Encode bitmap with quality lost to WebP byte array. Opcionally select 'quality'.
    /// byte[] <see cref="EncodeLossy"/>(Bitmap bmp, int quality, int speed, bool info = false) - Encode bitmap with quality lost to WebP byte array. Select 'quality' and 'speed'. 
    /// byte[] <see cref="EncodeLossless"/>(Bitmap bmp) - Encode bitmap without quality lost to WebP byte array. 
    /// byte[] <see cref="EncodeLossless"/>(Bitmap bmp, int speed, bool info = false) - Encode bitmap without quality lost to WebP byte array. Select 'speed'. 
    /// byte[] <see cref="EncodeNearLossless"/>(Bitmap bmp, int quality, int speed = 9, bool info = false) - Encode bitmap with nearlossless to WebP byte array. Select 'quality' and 'speed'. 
    /// 
    /// Another functions:
    /// string <see cref="GetVersion"/> - Get the library version
    /// <see cref="GetInfo"/>(byte[] rawWebP, out int width, out int height, out bool has_alpha, out bool has_animation, out string format) - Get information of WEBP data
    /// float[] <see cref="GetPictureDistortion"/>(Bitmap source, Bitmap reference, int metric_type) - Get PSNR, SSIM or LSIM distortion metric between two pictures
    /// </code>
    /// </summary>
    public sealed class WebPFormat : IDisposable
    {
